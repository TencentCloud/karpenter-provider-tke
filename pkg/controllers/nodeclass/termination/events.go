...
func WaitingOnNodeClaimTerminationEvent(nodeClass *api.TKEMachineNodeClass, names []string) events.Event {
	return events.Event{
		InvolvedObject: nodeClass,
		Type:           corev1.EventTypeNormal,
		Reason:         "WaitingOnNodeClaimTermination",
		Message:        fmt.Sprintf("Waiting on NodeClaim termination for %s", util.PrettySlice(names, 5)),
