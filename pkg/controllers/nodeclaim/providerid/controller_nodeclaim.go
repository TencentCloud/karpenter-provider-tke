...
type ControllerNodeClaim struct {
	kubeClient client.Client
}

func NewControllerNodeClaim(kubeClient client.Client) *ControllerNodeClaim {
	return &ControllerNodeClaim{
		kubeClient: kubeClient,
	}
}

func (c *ControllerNodeClaim) Reconcile(ctx context.Context, nodeClaim *v1.NodeClaim) (reconcile.Result, error) {
	...
	if err != nil {
		return reconcile.Result{RequeueAfter: 5 * time.Second}, fmt.Errorf("listing machines failed: %v", err)
	}
	...
	if len(lo.FromPtr(machineList.Items[0].Spec.ProviderID)) == 0 {
		...
		nodeClaim.Status.ProviderID = fmt.Sprintf("placeholder-%s", machineList.Items[0].Name)
		// return reconcile.Result{RequeueAfter: 5 * time.Second}, nil
	} else {
...
func (c *ControllerNodeClaim) Register(_ context.Context, m manager.Manager) error {
